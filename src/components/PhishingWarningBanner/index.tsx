...
const PhishingWarningBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  const warningTextAsParts = useMemo(() => {
    const warningText = t("please make sure you're visiting KITASWAP - check the URL carefully.")
    return warningText.split(/(https:\/\/KITASWAP.pro)/g)
  }, [t])
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          as="span"
          bold={text === 'KITASWAP'}
          color={text === 'KITASWAP' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
