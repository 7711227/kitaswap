...
const TotalStaked: React.FC<React.PropsWithChildren<MultiplierProps>> = ({ multiplier }) => {
  ...
  const tooltipContent = (
    <>
      <Text>
        {t(
          'The Multiplier represents the proportion of TINT rewards each farm receives, as a proportion of the TINT produced each block.',
        )}
      </Text>
      <Text my="24px">
        {t('For example, if a 1x farm received 1 TINT per block, a 40x farm would receive 40 TINT per block.')}
      </Text>
