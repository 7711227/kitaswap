...
const stepsConfigBuilder = ({ t, userInfos, eventInfos, userStatus, account, theme, cakeBalance }: EventStepsType) => {
  ...
  return [
    ...
    },
    {
      ...
      title: t('Hold TINT'),
      bodyText: [t('Each NFT costs TINT to mint. Remember you also need BNB to cover transaction fees too!')],
      buttons: (
        <>
          <Button
            as="a"
            href="/swap?outputCurrency=0x527ad1d5bcee13a2952d1832b18011649a78266b&chainId=56"
            target="_blank"
            width="100%"
             style={{background:'#7a65a4 !important'}}
          >
          <span style={{color:'#111'}}>  {t('Buy TINT')}</span>
          </Button>
        </>
      ),
    },
