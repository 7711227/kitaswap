      ...
      {Number.isFinite(lockedCakeBalance) && (
        <Box>
          <Text>
            {isBoostingExpired
              ? t(
                  'Your vTINT boosting was expired at the snapshot block. Renew your fixed-term staking position to activate the boost for future voting proposals.',
                )
              : t(
                  'Voting power is calculated using the staking amount and remaining staking duration of the fixed-term TINT staking position at the block.',
                )}
          </Text>
          <Text bold m="10px 0">
            {`${t('TINT locked:')} ${formatNumber(lockedCakeBalance, 0, 2)}`}
          </Text>
  ...
  return (
    <ModalInner mb="0">
      <Text as="p" mb="24px" fontSize="14px" color="textSubtle">
        {t(
          'Your voting power is determined by the amount of CAKE you held and the remaining duration on the fixed-term staking position (if you have one) at the block detailed below. TINT held in other places does not contribute to your voting power.',
        )}
      </Text>
                ...
                <Flex mt="10px" flexDirection="column" alignItems="flex-end">
                  ...
                  <Flex>
                    ...
                    <Text ml="4px" color={isBoostingExpired ? 'warning' : 'secondary'} fontSize="12px">
                      {isBoostingExpired ? t('Boosting Expired') : t('Boosted by TINT')}
                    </Text>
                  </Flex>
                </Flex>
              </FixedTermCardInner>
